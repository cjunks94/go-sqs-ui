...
test.describe('Message Export', () => {
    ...
    if (downloadPath) {
      ...
      data.messages.forEach((msg: any) => {
        const hasError =
          ...
          Object.values(msg.attributes || {}).some((v: any) => v.toString().toLowerCase().includes('error'));
  ...
  test('should show export format options', async () => {
    ...
    if (formatOptions.length > 0) {
      ...
      const optionTexts = await Promise.all(formatOptions.map((opt) => opt.textContent()));
  ...
  test('should preserve message order in export', async () => {
    ...
    const uiMessageIds = await page.$$eval('.message-item', (items) =>
      items.map((item) => item.getAttribute('data-message-id') || item.querySelector('.message-id')?.textContent)
    );
