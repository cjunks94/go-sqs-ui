...
test.describe('Queue Browser Mode', () => {
  ...
  test('should navigate to next page', async () => {
    ...
    const initialMessages = await page.$$eval('.browser-message-item', (items) =>
      items.map((item) => item.querySelector('.browser-message-id')?.textContent)
    );
    ...
    if (nextButton) {
      ...
      if (!isDisabled) {
        ...
        // Get new message IDs
        const newMessages = await page.$$eval('.browser-message-item', (items) =>
          items.map((item) => item.querySelector('.browser-message-id')?.textContent)
        );
