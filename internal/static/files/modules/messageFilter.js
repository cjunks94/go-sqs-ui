...
export class MessageFilter {
  ...
  matchesTextFilter(message, textFilter) {
    ...
    if (message.attributes) {
      for (const [key, value] of Object.entries(message.attributes)) {
        if (key.toLowerCase().includes(searchText) || value.toLowerCase().includes(searchText)) {
          return true;
        }
      }
    }
  ...
  handleDeleteSelected() {
    ...
    toast.confirm(`Delete ${selectedMessages.length} selected messages?`, () => {
      // Trigger batch delete event
      window.dispatchEvent(
        new CustomEvent('batchDelete', {
  ...
  handleRetrySelected() {
    ...
    toast.confirm(`Retry ${selectedMessages.length} selected messages?`, () => {
      // Trigger batch retry event
      window.dispatchEvent(
        new CustomEvent('batchRetry', {
          detail: { messageIds: selectedMessages },
        })
      );
    });
