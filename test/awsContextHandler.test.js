...
import { AWSContextHandler } from '../internal/static/files/modules/awsContextHandler.js';
...
describe('AWSContextHandler', () => {
  ...
  describe('Live AWS Context', () => {
    ...
    it('should display live AWS context with session credentials', async () => {
      ...
      const accountRow = Array.from(rows).find(
        (row) => row.querySelector('.aws-context-label').textContent === 'Account'
      );
  ...
  describe('UI Rendering', () => {
    ...
    it('should skip N/A values in rendering', async () => {
      ...
      const labels = Array.from(rows).map((row) => row.querySelector('.aws-context-label').textContent);
