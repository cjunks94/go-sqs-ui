...
test.describe('Keyboard Navigation', () => {
  ...
  test('should navigate messages with j/k keys', async () => {
    ...
    if (firstMessage) {
      const hasFocus = await firstMessage.evaluate(
        (el) => el.classList.contains('keyboard-focused') || el === document.activeElement
      );
      ...
      const secondHasFocus = await secondMessage?.evaluate(
        (el) => el.classList.contains('keyboard-focused') || el === document.activeElement
      );
      ...
      const firstHasFocusAgain = await firstMessage.evaluate(
        (el) => el.classList.contains('keyboard-focused') || el === document.activeElement
      );
  ...
  test('should jump to top with gg keys', async () => {
    ...
    const firstMessage = await page.$('.message-item:first-child');
    if (firstMessage) {
      const hasFocus = await firstMessage.evaluate((el) => el.classList.contains('keyboard-focused'));
      expect(hasFocus).toBe(true);
    }
  });
  ...
  test('should jump to bottom with G key', async () => {
    ...
    if (lastMessage) {
      const hasFocus = await lastMessage.evaluate((el) => el.classList.contains('keyboard-focused'));
      expect(hasFocus).toBe(true);
    }
  });
